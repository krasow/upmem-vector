...
#define INSTANTIATE_BINARY_OP(T, OP) \
    template dpu_vector<T> OP<T>(const dpu_vector<T>& lhs, const dpu_vector<T>& rhs);
#define INSTANTIATE_UNARY_OP(T, OP) \
    template dpu_vector<T> OP<T>(const dpu_vector<T>& vec);
#define INSTANTIATE_TO_CPU(T) \
    template std::vector<T> dpu_vector<T>::to_cpu();
#define INSTANTIATE_FROM_CPU(T) \
    template dpu_vector<T> dpu_vector<T>::from_cpu(std::vector<T>& cpu_vec, LOGGER_ARGS);
