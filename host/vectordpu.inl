...
// ============================
template <typename T>
dpu_vector<T>::dpu_vector(uint32_t n, LOGGER_ARGS) // name and loc from LOGGER_ARGS
    : size_(n), debug_name(name.data()), debug_file(loc.file_name()), debug_line(loc.line())
{ 
...
template <typename T>
dpu_vector<T> dpu_vector<T>::from_cpu(std::vector<T>& cpu_vec, LOGGER_ARGS)
{   
    dpu_vector<T> vec(cpu_vec.size(), name, loc); // name and loc from LOGGER_ARGS
    ...
    DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, 
        ...
        std::cout << "[debug-help] Transferred " << cpu_vec.size() << " elements to DPUs" << std::endl;
    #endif 
    return vec;
}
