...
MappedChain map_independent_chain(
  ...
  for (size_t k = 0; k < chain_rpn.size(); ++k) {
    ...
    if (op == OP_PUSH_INPUT) {
      ...
      append_token_with_inline_bytes(chain_rpn, k, mapped.rpn);
...
std::string horizontal_slice_name(const FusionRpnSummary& summary) {
...
void log_horizontal_fusion(const std::shared_ptr<Event>& last,
  ...
  log_fusion_header(log, "horizontal fusion", "independent_same_length", last,
                    e, before);
  log.second() << "existing expr: "
               << fusion_rpn_expr_preview(ops.target_rpn, 1, 90);
  log.second() << "new expr: " << fusion_rpn_expr_preview(ops.chain_rpn, 1, 90);
  log_fused_kernel_tail(log, last, summary);
#else
  (void)last;
  (void)e;
  (void)before;
...
bool EventQueue::try_hfuse(std::shared_ptr<Event> last,
  ...
  MappedChain mapped = map_independent_chain(
